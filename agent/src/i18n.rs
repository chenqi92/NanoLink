...
pub fn t(key: &str, lang: Lang) -> &'static str {
    match (key, lang) {
        ...
        ("config.realtime_range", Lang::Zh) => "值必须在 100 到 3600000 毫秒之间",
        ("config.realtime_range", Lang::En) => "Value must be between 100 and 3600000 ms",
        ("config.buffer_range", Lang::Zh) => "值必须在 10 到 100000 之间",
        ("config.buffer_range", Lang::En) => "Value must be between 10 and 100000",
        ("config.heartbeat_range", Lang::Zh) => "值必须在 1 到 3600 秒之间",
        ("config.heartbeat_range", Lang::En) => "Value must be between 1 and 3600 seconds",
        ...
        ("service.windows_start_hint", Lang::Zh) => "使用“sc start NanoLinkAgent”启动服务。",
        ("service.windows_start_hint", Lang::En) => {
            "Use 'sc start NanoLinkAgent' to start the service."
        }
        ...
        ("service.root_required", Lang::Zh) => "需要 root 权限，请使用 sudo 运行",
        ("service.root_required", Lang::En) => "Root permission required. Run with sudo.",
        ("service.invalid_executable_path", Lang::Zh) => "可执行文件路径包含无效字符",
        ("service.invalid_executable_path", Lang::En) => "Invalid characters in executable path",
        ("service.invalid_config_path", Lang::Zh) => "配置文件路径包含无效字符",
        ("service.invalid_config_path", Lang::En) => "Invalid characters in config path",
        ("service.write_file_failed", Lang::Zh) => "写入服务文件失败",
        ("service.write_file_failed", Lang::En) => "Failed to write service file",
        ("service.state_stopped", Lang::Zh) => "已停止",
        ("service.state_stopped", Lang::En) => "Stopped",
        ("service.state_starting", Lang::Zh) => "正在启动...",
        ("service.state_starting", Lang::En) => "Starting...",
        ("service.state_stopping", Lang::Zh) => "正在停止...",
        ("service.state_stopping", Lang::En) => "Stopping...",
        ("service.state_running", Lang::Zh) => "运行中",
        ("service.state_running", Lang::En) => "Running",
        ("service.state_resuming", Lang::Zh) => "正在恢复...",
        ("service.state_resuming", Lang::En) => "Resuming...",
        ("service.state_pausing", Lang::Zh) => "正在暂停...",
        ("service.state_pausing", Lang::En) => "Pausing...",
        ("service.state_paused", Lang::Zh) => "已暂停",
        ("service.state_paused", Lang::En) => "Paused",
        ...
        ("diag.low_space", Lang::Zh) => "可用空间不足 10%",
        ("diag.low_space", Lang::En) => "Less than 10% free",
