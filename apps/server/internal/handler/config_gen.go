...
func (h *ConfigGenHandler) GenerateAddServerCommand(c *gin.Context) {
	...
	unixCmd := fmt.Sprintf(
		...
		shellQuote(cliHost), port, shellQuote(req.Token), req.Permission, req.TLSVerify, req.TLSVerify,
	)

	windowsCmd := fmt.Sprintf(
		`nanolink-agent.exe server add --host %s --port %d --token %s --permission %d --tls-enabled=%t --tls-verify=%t`,
		windowsQuote(cliHost), port, windowsQuote(req.Token), req.Permission, req.TLSVerify, req.TLSVerify,
	)
...
logging:
  level: info
  audit_enabled: true
`, hostnameConfig, yamlString(host), port, yamlString(token), req.Permission, req.TLSVerify, req.TLSVerify, shellConfig)
}
